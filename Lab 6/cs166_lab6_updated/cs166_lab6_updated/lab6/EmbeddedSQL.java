...
public class EmbeddedSQL {
   ...
   public static void Query1(EmbeddedSQL esql){
      try{
         String query = "SELECT S.sid, S.sname, COUNT(C.pid) AS num_parts\n" +
             "FROM suppliers S\n" +
   ...
   public static void Query3(EmbeddedSQL esql){
      try{
         String query = "SELECT S.sname, COUNT(C.pid) AS num_green_parts\n" +
            "FROM suppliers S\n" +
            "JOIN catalog C on S.sid = C.sid\n" +
   ...
   public static void Query4(EmbeddedSQL esql){
      try{
         String query = "SELECT S.sname, MAX(C.pid) AS most_expensive_part\n" +
            "FROM suppliers S\n" +
            "JOIN catalog C on S.sid = C.sid\n" +
            "JOIN parts P on P.pid = C.pid\n" +
